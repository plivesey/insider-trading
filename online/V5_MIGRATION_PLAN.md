...
- [x] Full build (shared → backend → frontend): zero TypeScript errors.
- [x] Full `npm test` green: root 63/63, backend 170/170, frontend 27/27.
- [x] `verifyE2E.ts`/`verifyHttp.ts` rewritten for V5 and passing. This surfaced a real production bug along the way: `replayFromLog` (`backend/src/domain/replay.ts`) never bootstrapped the setup draft the way `ServerHub.startGame` does (`createGameState` alone doesn't call `beginDraft` -- only `advance()` does), so replaying any V5 game's log would fail on its very first `op_prompt_response`. Zero test coverage had caught this until now. Fixed, and added `backend/tests/integration/replay.test.ts` for permanent coverage. Also discovered along the way: `ProjectedGameState` never exposes `turnPhase` to the client at all (by design -- the frontend already detects "setup draft in progress" purely from a pending `setup_draft_pick` prompt, never from `turnPhase`); both verify scripts were updated to do the same instead of asserting on a field the wire protocol doesn't carry.
- [x] Manual multi-tab local playtest: two genuinely independent browser sessions (separate Playwright browser contexts, so separate cookie jars -- a same-context new tab would have shared Anna's identity), joined as "Anna" and "Ben" with no bots, drove the full 3-round setup draft on both sides, then one player started an auction and the other bid/passed on it, all propagating correctly over WebSocket in real time between the two independent sessions. Confirms the "play against friends online" path end-to-end, not just the bot-driven path. Hit the same pre-existing (non-V5) spectator-flash quirk noted in Phase 11 -- this time on the joining player's tab rather than the starter's -- confirming it's a general post-`start` cookie/WS-race, not specific to whoever clicks Start; a reload fixed it immediately, as before.
- [x] Dead-code sweep: `tryFireBlackMarketTrigger`, `startSideAuction`, `sideAuctionTip`, `resumePhase`, `MIN_TIPS`, `CLASSIC_RULES`, the Hot Tip/`peek_cards.json` loader path, and `final_tip_play_choice` are all already fully gone (removed during earlier phases) -- confirmed via a repo-wide grep, nothing left to delete. One vestigial trace remains: `hotTipThreshold`, a bot-tuning parameter that's no longer read by any decision logic. Left in place deliberately: it's part of the frozen positional parameter-vector schema (`PARAM_SPECS`/`encodeParams`) that the trained-net optimizer depends on, and removing a slot would shift every subsequent parameter's vector position, silently invalidating any previously-trained/saved profiles -- exactly the kind of frozen-schema change the plan's Phase 8 note says is out of scope for this project.
- [x] Root `CLAUDE.md` was already fully updated for V5 (done back in Phase 0) -- only its one stale line describing `online/` as "mid-migration" needed updating now that it's complete. `online/PLAN.md` and `online/PROGRESS_LOG.md` are the original V4 build's own plan/log; rather than rewriting that history, added a clear "superseded, see `online/V5_MIGRATION_PLAN.md`" banner to the top of each so a future reader doesn't mistake V4-era content for the current app.
- [x] Final Phase 11 browser-QA checklist re-run as closing sign-off: done via the multi-tab playtest above (fresh dev-server boot, zero console errors on the tracked session, full draft + auction flow across two real independent sessions).
- [x] Commit.

**V5 migration: all 12 phases complete.** The online app is a fully playable, tested V5 implementation: 63 root + 170 backend + 27 frontend automated tests green, zero TypeScript errors across all three workspaces, both verify scripts passing end-to-end, and live-browser-confirmed for both the bot-opponent path and the friends-online path. Two genuine design/rules edge cases were surfaced during testing and documented rather than silently patched: the private-market-movement-card stagnation risk (`v5_tuning_notes.md` #11, mitigated for bots with a bounded heuristic fallback) and two small pre-existing (non-V5) UX papercuts around session/cookie timing on game start (noted in Phase 11, not fixed -- out of migration scope).

**Post-migration update (2026-09-14): the value net has since been retrained.** Growing the frozen 40-slot feature schema (to give bots learned judgment over the new cards, dice bag, and drafting) is still out of scope and left for a separate future project -- but a same-schema retrain of the *existing* stock-valuation net turned out to be necessary sooner than expected: several of its input features carried silently stale normalization from V4 (starting cash $30→$25, loan cap 3→2, and — most significantly — a deck-size feature calibrated to V4's small curated tip deck now reading 2-5.5x larger against V5's much bigger merged event deck, well outside anything the net was ever trained on). See `.claude/skills/retrain-bot-value-net/SKILL.md` for the full writeup and the diagnostic process, and `scripts/compareChampions.ts` for the head-to-head harness. Retrained via `scripts/trainSelfPlay.ts`'s champion-vs-challenger self-play (6 promoted rounds against the V4-trained champion, ~2 minutes total on this hardware), validated at +34.6% average edge over the previous champion across 2-5 players (min +18.8% at 5p, all 95% CIs clear of fair share over 400 games/count), and promoted to production `nets/champion.json`. Heuristic bots remain the baseline for every V5-specific decision point (goals, action cards, drafting) — only stock valuation is net-driven, unchanged from before.
